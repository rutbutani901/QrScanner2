...
import scanner.qrcodescanner.qrcodereader.barcodescanner.barcodereader.databinding.ActivityCreatePhoneCodeBinding
...
class CreatePhoneCode : AppCompatActivity() {

    lateinit var binding:ActivityCreatePhoneCodeBinding
...
//                application.setTheme(R.style.PinkTheme)
        ...
        binding= ActivityCreatePhoneCodeBinding.inflate(layoutInflater)
    ...
    fun gotoNextScrren(){
        val phone= binding.input.text.toString().trim()
        ...
        if(phone != "")
        {
            ...
            intent.putExtra("barcodeValue", phone)
            intent.putExtra("barcodeType", 4)//4 is for phone, barcodeType
